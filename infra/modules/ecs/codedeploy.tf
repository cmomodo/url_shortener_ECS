...
resource "aws_codedeploy_deployment_group" "dashboard" {
  ...
  load_balancer_info {
    target_group_pair_info {
      prod_traffic_route {
        listener_arns = [aws_lb_listener_rule.dashboard.arn]
      }
...
resource "aws_codedeploy_deployment_group" "worker" {
  app_name               = aws_codedeploy_app.worker.name
  deployment_group_name  = "url-shortener-worker"
  service_role_arn       = var.codedeploy_service_role_arn
  deployment_config_name = "CodeDeployDefault.ECSLinear10PercentEvery1Minutes"
  ...
  load_balancer_info {
    target_group_pair_info {
      prod_traffic_route {
        listener_arns = [aws_lb_listener.worker_prod.arn]
      }

      test_traffic_route {
        listener_arns = [aws_lb_listener.worker_test.arn]
      }

      target_group {
        name = aws_lb_target_group.worker.name
      }

      target_group {
        name = aws_lb_target_group.worker_green.name
      }
    }
  }
}
